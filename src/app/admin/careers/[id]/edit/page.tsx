  ...
  const fetchCareer = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          title: career.title || '',
          department: career.department || 'Creative Arts',
          location: career.location || 'Badlapur, Maharashtra, India',
          ...
          qualifications: career.qualifications?.length ? career.qualifications : [''],
          benefits: career.benefits?.length ? career.benefits : [''],
          salary: {
            min: career.salary?.min?.toString() || '',
            max: career.salary?.max?.toString() || '',
            currency: career.salary?.currency || 'INR',
            period: career.salary?.period || 'yearly'
          },
          ...
          postedBy: career.postedBy || 'HR Team'
        })
      } else {
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">Update career opportunity details</p>
          </div>
        </div>
      </div>
